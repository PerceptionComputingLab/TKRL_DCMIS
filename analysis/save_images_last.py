...
def visualize_prediction(image, target, predicted, dice_score, title, save_path=None, image_path=None):
    """
    可视化预测结果并保存图片（可选）
    ...
    if image_path is not None:
        # save image to disk
        plt.title(title)
        plt.imshow(np.rot90(image, 3), cmap="gray")
        plt.axis("off")
        plt.tight_layout()
        if save_path:
...
def process_experiment(config, approach, dataset_, out_dir):
    ...
    for i, loader in enumerate(test_dataloader):
        ...
        for _, feature in enumerate(features):
            ...
            print(f"Metrics for approach {approach}, {dataset_} {i}: {metrics}")
            ...
            visualize_prediction(
                ...
                title=f"{approach} - {dataset_} {i}",
                save_path=os.path.join(out_dir, f"{dataset_}_{approach}_{i}_{dice_score:.4g}.png"),
                image_path=os.path.join(out_dir, f"{dataset_}_{approach}_{i}.png"),
            )
            break
...
def main():
    ...
    out_dir = os.path.join(abs_path, storage_path, "save_images_last")
