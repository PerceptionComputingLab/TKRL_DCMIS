...
def extract_model_outputs(agent, dataloader):
    ...
    with torch.no_grad():
        for data in tqdm(dataloader, desc=f"Extracting model outputs"):
            sample_data = {}
            ...
            for idx in range(len(sample_data["inputs"])):
                ...
                all_incorrect_entropy_values.append(avg_incorrect_entropy)
...
def process_experiment(config, approach, dataset_, out_dir):
    ...
    total_entropy_list = []
    mean_entropy_list = []
    for idx, best_state in enumerate(best_states):
        agent.restore_state(exp_run.paths["states"], best_state)
        if idx == 0:
            if config["approach"] == "seq":
                pass
            else:
                total_entropy_list.append(0)
                mean_entropy_list.append(0)

        all_incorrect_entropy_values = extract_model_outputs(agent, test_dataloader[0])
        total_entropy = np.sum(all_incorrect_entropy_values)
        mean_entropy = np.mean(all_incorrect_entropy_values)
...
def main():
    ...
    for dataset_ in datasets_:
        ...
        for approach in approaches:
            ...
            print(mean_entropy_list)

        plt.title(f"Mean Entropy for Dataset '{dataset_}'")
