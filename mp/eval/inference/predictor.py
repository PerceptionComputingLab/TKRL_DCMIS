...
class Predictor:
    ...
    def get_numpy_subject(self, subject_ix):
        r"""Get a numpy version of the subject."""
        path = self.instances[subject_ix].file_path

        x = np.load(path)["image"]
...
class Predictor2D(Predictor):
    ...
    def get_subject_prediction(self, agent, subject_ix, probabilities=False):
        ...
        x, y = self.get_numpy_subject(subject_ix)
        x = torch.from_numpy(x).to(agent.device)
        ...
        with torch.no_grad():
            for slice_idx in range(len(x)):
                inputs = x[slice_idx].float().unsqueeze(0).unsqueeze(0)
                inputs = agent.model.preprocess_input(inputs)

                if probabilities:
                    slice_pred_prob = agent.get_outputs(inputs).float()
                    slice_pred = torch.where(
                        slice_pred_prob > 0.5,
                        torch.ones_like(slice_pred_prob),
                        0 * slice_pred_prob,
                    )
...
class GridPredictor(Predictor):
    ...
    def get_subject_prediction(self, agent, subject_ix):
        ...
        grid_sampler = torchio.inference.GridSampler(
            sample=subject, patch_size=self.patch_size, patch_overlap=self.patch_overlap
        )
