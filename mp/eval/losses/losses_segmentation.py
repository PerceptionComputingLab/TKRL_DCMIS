...
class LossAKT(LossAbstract):
    ...
    def forward(self, output, target):
        ...
        # uncertainty: (batch_size, height, width)
        # error_predictions: (batch_size, 1, height, width)
...
class LossDice(LossAbstract):
    ...
    def __init__(self, smooth=1e-5, device="cuda:0"):
        super().__init__(device=device)
        self.smooth = smooth
        self.device = device
    ...
    def forward(self, output, target):
        ...
        return 1 - ((2.0 * intersection + self.smooth) / (output_flat.sum() + target_flat.sum() + self.smooth))
...
class LossClassWeighted(LossAbstract):
    ...
    def forward(self, output, target):
        ...
        for instance_output, instance_target in zip(output, target):
            ...
            for out_channel_output, out_channel_target, weight in zip(
                instance_output, instance_target, self.class_weights
            ):
                instance_loss += weight * self.loss(out_channel_output, out_channel_target)
    ...
    def get_evaluation_dict(self, output, target):
        ...
        for instance_output, instance_target in zip(output, target):
            for out_channel_output, out_channel_target, weight_ix in zip(
                instance_output, instance_target, range(len(weighted_loss_values))
            ):
