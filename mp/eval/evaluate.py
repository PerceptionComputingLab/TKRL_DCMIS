...
from mp.eval.metrics.mean_scores import get_mean_scores_with_mask
...
def ds_metrics(ds, agent, metrics):
    ...
    for instance_ix, instance in enumerate(ds.instances):
        subject_name = instance.name
        target = torch.from_numpy(np.load(instance.file_path)["label"]).to(agent.device).permute(1, 2, 0).unsqueeze(0)
        target = torch.where(target == ds.target, torch.ones([1]).to(agent.device), torch.zeros([1]).to(agent.device))
        ...
        if False:
            ...
            ax[0].imshow(target.squeeze().cpu()[:, :, -1])
            ax[1].imshow(pred.squeeze().cpu()[:, :, -1])
