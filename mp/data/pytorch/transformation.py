...
NORMALIZATION_STRATEGIES = {
    None: None,
    "rescaling": torchio.transforms.RescaleIntensity(out_min_max=(0, 1), percentiles=(0.1, 99.0)),
...
AUGMENTATION_STRATEGIES = {
    ...
    "standard": torchio.transforms.Compose(
        [
            ...
            torchio.transforms.RandomMotion(p=0.1, degrees=10, translation=10, num_transforms=2),
...
def label_to_channel(y, target_classes=1):
    zeros = torch.zeros(y.shape[1:], dtype=torch.float64)
    ones = torch.ones(y.shape[1:], dtype=torch.float64)

    foreground = torch.where(y == target_classes, ones, zeros)
    background = torch.where(y != target_classes, ones, zeros)
    target = torch.cat([background, foreground], dim=0)
    return target
...
def torchvision_rescaling(x, size=(3, 224, 224), resize=False):
    ...
    transform_ops.append(transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]))
