...
class TKRLAgent(SegmentationAgent):
    ...
    def save_recon_images(self, inputs, save_path, epoch):
        ...
        """
        self.model.get_features_variance_ranks(inputs)
        ...
        for i in range(len(inputs)):
            ...
            save_image(self.model.probes_mask[i], os.path.join(save_path, f"{epoch}-{i}_mask.png"))
            save_image(
                recom[i] * self.model.probes_mask[i] + inputs[i] * (1 - self.model.probes_mask[i]),
    ...
    def inference_probes_noise(self, config, save_path, dataset_index):
        ...
        for step in pbar:
            ...
            loss = -torch.mean(output) + total_variation_loss(
                self.probes_noise
            )  # + torch.norm(self.probes_noise, p=1)
    ...
    def perform_training_epoch(self, loss_f, train_dataloader, config, epoch, print_run_loss=False, save_path=""):
        ...
        for data in pbar:
            ...
            loss_distill = self.calculate_distillation_loss(inputs, outputs, targets)
    ...
    def calculate_distillation_loss(self, inputs, outputs, targets):
        """
        ...
        if self.model.backbone_old:
            ...
            if self.taggle:
                # Probe-augmented Class Distillation against Gaps
                self.taggle = False
                ...
                pcd_loss = self.multi_class_cross_entropy_no_softmax(outputs_noise, outputs_noise_old)
                return kd_loss + pcd_loss

            else:
                # Variance-guided Masked Autoencoder against Biases
                self.taggle = True
