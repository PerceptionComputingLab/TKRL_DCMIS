...
def process_dataset(data_type, datapath, group, new_dir, hw=192):
    """
    ...
    """
    mask_dir = os.path.join(datapath, group, data_type, "ROIs", "mask")
    image_dir = os.path.join(datapath, group, data_type, "ROIs", "image")

    for patient in os.listdir(mask_dir):
        mask_path = os.path.join(mask_dir, patient)
        ...
        target = os.path.join(new_dir, patient.replace(".png", ".npz"))
...
def main():
    ...
    """
    datapath = os.path.join(source_data_path, "Fundus")
    ...
    for group in os.listdir(datapath):
        ...
        process_dataset("train", datapath, group, new_dir, hw)
        process_dataset("test", datapath, group, new_dir, hw)
